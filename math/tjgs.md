...
- [微分中值定理与导数的应用](#微分中值定理与导数的应用)
  ...
  - [曲率](#曲率)
    ...
    - [曲率及其计算公式](#曲率及其计算公式)
    - [曲率圆与曲率半径](#曲率圆与曲率半径)
    - [曲率中心的计算公式, 渐屈线与渐伸线](#曲率中心的计算公式-渐屈线与渐伸线)
  - [方程的近似解](#方程的近似解)
    - [二分法](#二分法)
    - [切线法](#切线法)
    - [割线法](#割线法)
...
还要说明两点:
* 如果 $f(x)$ 在区间 $I$ 上有原函数, 即有一个函数 $F(x)$ , 是对于任一 $x\in I$ 都有 $F'(x)=f(x)$, 那么对任何常数 $C$, 显然也有
```math
[F(x)+C]'=f(x)
```
即对于任何常数 $C$, 函数 $F(x)+C$ 也是 $f(x)$ 的原函数, 那么 $f(x)$ 的其他函数与 $F(x)$ 有什么关系?
